...
else combo --mode train --cuda_device 0 --num_epochs 100 --pretrained_transformer_name cl-tohoku/bert-base-japanese-whole-word-masking --training_data_path ja_gsd_modern.conllu --targets deprel,head,upostag --features token,char,xpostag,lemma
...
else sed 's/^\([1-9][0-9]	\)\([^	]*	\)\([^	]*	\)/\1\3\2/' ja_gsd_modern.conllu > ja_rev.conllu
     combo --mode train --cuda_device 0 --num_epochs 100 --pretrained_transformer_name cl-tohoku/bert-base-japanese-whole-word-masking --training_data_path ja_rev.conllu --targets deprel,head,upostag --features token,char,xpostag,lemma
